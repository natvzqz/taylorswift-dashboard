...
with header:
    ...
    > Disclaimer: esta es una app que está en construcción, la estoy usando para aprender y 
     ...
     si algo no funciona o el diseño no es el mas friendly. Gracias por su comprensión.""")
...
with basicstats:
    ...
    with col2:
            ...
            st.markdown("""
             > disclaimer: el botón de rola, aún no sirve para nada, pero próximamente funcionara""")
...
with audiofeatures:
    ...
    with col5:
        ...
        widget_cancion5 = st.selectbox('Cancion 1',options=canciones_album5,index = (len(canciones_album5)-1))
    ...
    with col6:
        ...
        widget_cancion6 = st.selectbox('Cancion 2',options=canciones_album6,index = (len(canciones_album6)-1))
    ...
    with col7:
        ...
        widget_cancion7 = st.selectbox('Cancion 3',options=canciones_album7,index = (len(canciones_album7)-1))
...
with sentAnalisis:
    st.header('Analisis de la Lyrica')
    ...
    with col10:
        ...
        st.write('Complejidad del album: ',round(rl_album,3))

        num_palabras_album = st.slider('Cuantas palabras quieres observar?',min_value=5,max_value=20,value=13)
    ...
    with col11:
        ...
        widget_cancion10 = st.selectbox('Escoge una canción:',options=canciones_album10,index = (len(canciones_album10)-1))
        ...
        num_palabras_cancion = st.slider('Cuantas palabras quieres ver?',min_value=5,max_value=20,value=13)
    ...
    with st.beta_expander("Cómo interpretar esto?:"):
            ...
            st.markdown("""
            - Complejidad: se refiere al num de palabras únicas entre el total de palabras """)
            st.markdown("""
            Siempre es muy interesante ver que palabras son las que más se repiten y como influyen en el sentimiento.""")
...
with conclusion:
    ...
    st.subheader('Revisa si tu canción fav ya fue analizado por swifting podcast.')
    
    widget_podcast = st.selectbox('álbum fav: ',options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore'],index = 0)
    df_podcast = df[df['album'] == widget_podcast]
    canciones_podcast= df_podcast['title'].unique().tolist()
    canciones_podcast.append('Todas')
    widget_cancion_podcast = st.selectbox('canción fav: ',options=canciones_podcast,index = (len(canciones_podcast)-1))
    df_cancion_podcast = df[df['title'] == widget_cancion_podcast]
     
    components.html('<iframe src="https://open.spotify.com/embed/episode/6PfsTfh015b93Y7Kqq8jau" width="100%" height="220" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>')
