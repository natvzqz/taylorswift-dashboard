# FIXES aplicados:
# 1. Eliminado: from altair.vegalite.v4.schema.channels import Key  (no existe en altair 5+)
# 2. Eliminado: from numpy.lib.function_base import disp              (removido en numpy 1.24+)
# 3. use_column_width='auto' → use_container_width=True              (deprecado en streamlit nuevo)
# 4. df.corr() → df.corr(numeric_only=True)                         (requerido en pandas 2.0+)
# 5. Agregado: nltk.download('punkt_tab')                            (requerido en nltk 3.9+)
# 6. df_album10 = df_album10.copy() antes de asignar 'sentiment'    (evita SettingWithCopyWarning)
...
with st.container():
    ...
    widget_album = sel_col.selectbox(
        'Elige un álbum',
        options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore'],
        index=0
    )
...
sobre las características de audio extraidas de Spotify""")
    ...
    with col3:
        st.write('Álbum: ', widget_album)
        columns_to_drop = ['title', 'artist', 'album', 'genre', 'lyrics', 'URL']
        # agregamos solo las que existan para no tirar KeyError
        optional_cols = ['Podcast', 'Pocast', 'pop', 'track_n', 'year']
        extra = [c for c in optional_cols if c in df_album.columns]
        df_albumcor = df_album.drop(columns=columns_to_drop + extra, errors='ignore')
        fig, ax = plt.subplots()
