# FIX 1: eliminada "from numpy.lib.function_base import disp"
# causaba crash inmediato en numpy >= 1.24
...
with header:
    ...
    # FIX 3: use_column_width='auto' deprecado → use_container_width=True
    st.image(header_image, width=800, use_container_width=True)
...
with dataset:
    ...
    st.markdown("""
- Contiene toda la discografía de Taylor, incluidas las Taylor Versions
...
with basicstats:
    ...
    widget_album = sel_col.selectbox(
        'Elige un álbum',
        options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore','Fearless (TV)','Red (TV)'],
        index=0
    )
    ...
    with col2:
        def set_image(widget_selection):
            ...
            elif widget_selection == 'Fearless (TV)':
                return 'https://i.ibb.co/pnh8hn8/Fearless-TV.jpg'
            else:
                return 'https://i.ibb.co/rfBD43b/Red-TV.jpg'

        imagen = set_image(widget_album)
        st.image(imagen, width=300, use_container_width=True)

        st.markdown("""
> disclaimer: este botón de rola, aún no sirve para nada, pero próximamente funcionara""")

        with st.expander("Ver explicación:"):
            st.markdown("Todos lo anterior esta expresado en promedios")
            st.markdown("Entre más grande sea el valor más describe esa característica al álbum")
...
sobre las características de audio extraidas de Spotify""")
    ...
    with col3:
        st.write('Álbum: ', widget_album)
        # FIX 2: quitado 'main_discography' de esta lista
        # ya fue dropeado antes en df.drop(columns=['sel','main_discography'])
        # y tiraba KeyError
        df_albumcor = df_album.drop(columns=['track_n','year','title','artist','album','genre','lyrics','pop','Pocast','URL'])
        fig, ax = plt.subplots()
        sns.heatmap(df_albumcor.corr(numeric_only=True), ax=ax)
        st.write(fig)
...
- dnce vs energy""")
    ...
    with col5:
        widget_album5 = st.selectbox('álbum 1: ', options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore','Fearless (TV)','Red (TV)'], index=0)
    ...
    with col6:
        widget_album6 = st.selectbox('álbum 2: ', options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore','Fearless (TV)','Red (TV)'], index=0)
    ...
    with col7:
        widget_album7 = st.selectbox('álbum 3: ', options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore','Fearless (TV)','Red (TV)'], index=0)
...
y si no fuera por su **story telling** y **lyrics**, ella seria una mas del montón.""")
    ...
    with col8:
        ...
        st.image(diary_image, width=200, use_container_width=True)
...
También quitamos mayúsculas y todos los signos de puntuación.""")
    ...
    import nltk
    nltk.download('punkt')
    # FIX 4: NLTK >= 3.9 requiere también punkt_tab
    ...
    with col10:
        widget_album10 = st.selectbox('Escoge un álbum: ', options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore','Fearless (TV)','Red (TV)'], index=0)
        df_album10 = df[df['album'] == widget_album10]

        data_album = df_album10['lyrics'].tolist()
        texto_album = []
        for x in range(0, len(data_album)):
            token_1_album = data_album[x].lower()
            token_2_album = nltk.regexp_tokenize(token_1_album, pattern)
    ...
    with col11:
        ...
        for x in range(0, len(data_cancion)):
            token_1_cancion = data_cancion[x].lower()
            token_2_cancion = nltk.regexp_tokenize(token_1_cancion, pattern)
    ...
    with st.expander("Cómo interpretar esto?:"):
        st.markdown("- Palabras únicas: es el total de palabras que no se repiten")
        st.markdown("- Complejidad: se refiere al numero de palabras únicas entre el total de palabras")
        st.markdown("Siempre es muy interesante ver que palabras son las que más se repiten y cómo influyen en el sentimiento.")
...
para así encontrar el sentimiento general de las canciones""")
    ...
    def sentiment_analyzer_scores_album(lyric):
        ...
        return score_album['compound']

    df_album10 = df_album10.copy()
    df_album10['sentiment'] = df_album10['lyrics'].apply(sentiment_analyzer_scores_album)
...
with Reverse_Search:
    ...
    st.markdown("Para esto haremos una búsqueda inversa, en la cual puedes escribir una palabra y te dirá en que canciones la dice:")
    ...
    st.markdown("Literalmente podría pasar horas encontrando relaciones entre canciones.")
    st.markdown("""Y es por eso que considero que esta parte es una clave importantísima dentro del análisis de sentimientos, por que nos da una idea clave del sentido que le da Taylor Swift a las palabras.
Es decir, puede ser totalmente contrario a lo que nos arroja la variable ***compund*** que era el promedio entre positivo, negativo y neutro.""")
    st.markdown("Por ejemplo, si buscas happy, que es una palabra calificada con sentimiento positivo, observaras que se encuentra en canciones no tan positivas o canciones que en definitivo son tristes.")
...
las entonaciones, las constantes metáforas y relaciones a ciertas horas del día o colores específicos, a las personas que te recuerda, en fin, podría pasar hoooooras hablando sobre cada una de estas canciones""")
    ...
    widget_podcast = st.selectbox('álbum fav: ', options=['Taylor Swift','Fearless','Speak Now','Red','1989','reputation','Lover','folklore','evermore','Fearless (TV)','Red (TV)'], index=8)
